(function(){
  ...
  function selectedVfd(){const idx=Number($('solarVfd')?.dataset.globalIndex);return Number.isInteger(idx)&&idx>=0?vfds[idx]||null:null;}
  function syncBudgetEquipment(d){
    if(!d||$('equipmentMode')?.value!=='solar_vfd')return d;
    const v=selectedVfd();
    if(!v)return d;
    d.solar_vfd={...v,is_solar_vfd:true,quantity:1,required_kw:Number($('solarVfdRequiredInput')?.value||required()||0)};
    d.inverter=null;
    return d;
  }
  function wrapBudget(){
    if(typeof window.buildBudgetFromDesign==='function'&&!window.buildBudgetFromDesign.__vfdBudgetWrapped){
      const orig=window.buildBudgetFromDesign;
      const w=function(d){return orig(syncBudgetEquipment(d));};
      w.__vfdBudgetWrapped=true;
      window.buildBudgetFromDesign=w;
    }
  }
  ...
  function sync(){const mode=$('equipmentMode')?.value||'inverter',on=mode==='solar_vfd';const h2=$('inverter')?.closest('section.card')?.querySelector('h2');if(h2)h2.textContent=on?'7. Inversor / Variador solar':'7. Inversor';$('inverter').style.display=on?'none':'';const lbl=$('inverterTechnicalLabel')||$('inverter').previousElementSibling;if(lbl)lbl.style.display=on?'none':'';$('inverterTechCard')?.style.setProperty('display',on?'none':'');$('inverterTechStatus')?.style.setProperty('display',on?'none':'');$('inverterDatasheet')?.style.setProperty('display',on?'none':'');$('solarVfdBlock').style.display=on?'block':'none';if(on)renderVfd();try{localStorage.setItem(STORAGE,mode);}catch(e){}}
  function install(){inject();if(!$('equipmentMode'))return;fetch('data/solar_vfd.json').then(r=>r.json()).then(d=>{vfds=Array.isArray(d)?d:[];const saved=localStorage.getItem(STORAGE);if(saved==='solar_vfd')$('equipmentMode').value='solar_vfd';sync();wrapBudget();}).catch(()=>{vfds=[];sync();wrapBudget();});$('equipmentMode').addEventListener('change',()=>{sync();wrapBudget();});$('solarVfd').addEventListener('change',()=>{const cs=vfds.filter(v=>v.phase_class===($('systemPhase')?.value||'TRIFASICO'));const v=cs[Number($('solarVfd').value)];tech(v,required());});$('systemPhase')?.addEventListener('change',()=>{if($('equipmentMode')?.value==='solar_vfd')renderVfd();});document.addEventListener('input',e=>{if($('equipmentMode')?.value==='solar_vfd'&&e.target.closest('#consumptionMatrixBody'))renderVfd();});}
  ...
  function boot(){inject();wrapProjectState();install();setTimeout(()=>{wrapProjectState();wrapBudget();},0);}
