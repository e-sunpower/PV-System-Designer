(function(){
 ...
 function syncStructure(){try{if(typeof window.updateStructureTechnical==='function')window.updateStructureTechnical();}catch(e){}}
 const oldSetState=window.setProjectFormState;if(typeof oldSetState==='function')window.setProjectFormState=function(state){const r=oldSetState.apply(this,arguments);try{const saved=state?.fields?.consumptionMatrix;if(Array.isArray(saved)&&saved.length&&typeof window.__esunConsumptionMatrixSetRows==='function')window.__esunConsumptionMatrixSetRows(saved);else if(typeof window.__esunConsumptionMatrixRerender==='function')window.__esunConsumptionMatrixRerender();setTimeout(()=>{try{syncStructure();if(typeof window.updateConsumption==='function')window.updateConsumption();if(typeof window.updateBatterySelection==='function')window.updateBatterySelection();if(typeof window.syncBatteryBudgetRow==='function')window.syncBatteryBudgetRow();if(typeof window.updateBatteryPreview==='function')window.updateBatteryPreview();refreshBackupButtons();reorderSections();}catch(e){}},0);}catch(e){}return r;};
